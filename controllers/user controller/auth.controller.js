...
const sendOtp = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ success: true, message: "OTP has been send successfully" });
  } catch (error) {
    ...
    res
      ...
      .json({
        success: false,
        message: "Something went wrong while sending otp for signup",
      });
  }
};
...
const verifyOtp = (req, res) => {
  ...
  if (otpStore[email] && otpStore[email].OTP === Number(code)) {
...
const signUpUser = async (req, res) => {
  try {
    ...
    if (!otpStore[email] || otpStore[email].verified == false) {
      console.log("Email is not verified for signup"); 
      return res.status(400).json({success:false, message:"Email is not verified for signup"})
    }
